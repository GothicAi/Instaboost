...
def combined_roidb_for_training(dataset_names, proposal_files, img_id, new_anns, coco):
    """Load and concatenate roidbs for one or more datasets, along with optional
    object proposals. The roidb entries are then prepared for use in training,
    which involves caching certain types of metadata for each roidb entry.
    """
    def get_roidb(img_id, new_anns, coco, dataset_name, proposal_file):
        ds = JsonDataset(dataset_name, img_id, new_anns, coco)
        roidb = ds.get_roidb(
            gt=True,
    ...
    roidbs = [get_roidb(img_id, new_anns, coco, *args) for args in zip(dataset_names, proposal_files)]
    roidb = roidbs[0]
    for r in roidbs[1:]:
        roidb.extend(r)
